...
from gravity.app import ui
from gravity.modules import writter
from gravity.modules import defaults
...
def main() -> None:
    ...
    writter.board_settings(
        ...
        board_settings = ",".join([f"{name}={value}" for name, value in defaults.APP.items()])
    )
